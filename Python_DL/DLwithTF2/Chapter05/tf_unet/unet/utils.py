...
def plot_prediction(x_test, y_test, prediction, save=False):
    ...
    fig, ax = plt.subplot(test_size, 3, figsize=(12, 12), sharey=True, sharex=True)
...
def to_rgb(img):
    """ Convert the given array into a RGB image

    Arguments:
        img : the array [nx, ny, channels] -- The array to convert
    Returns:
...
def crop_to_shape(data, shape):
    """ Copy the array to the given image shape by removing the border

    Arguments:
        data : array -- The array to crop
        shape : array -- The target shape
    """
