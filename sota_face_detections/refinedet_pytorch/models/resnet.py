...
class PyramidFeatures(nn.Module):
    def __init__(self, C2_size, C3_size, C4_size, C5_size, feature_size=256):
        ...
        self.P3_1 = nn.Conv2d(C5_size, feature_size, kernel_size=1, stride=1, padding=0)
        ...
        self.tcb = TCB(feature_size=feature_size)
    ...
    def forward(self, inputs):
        ...
        P3_upsampled_x = self.P3_upsample(P3_x)
        P3_x = self.P3_2(P3_x)
...
class RegressionModel(nn.Module):
    def __init__(self, num_features_in, num_anchors=9, feature_size=256):
        ...
        #self.free_anchor = nn.Conv2d(feature_size, 4, kernel_size=3, padding=1)
        self.base_anchor = nn.Conv2d(feature_size, num_anchors * 4, kernel_size=3, padding=1)
...
class ClassificationModel(nn.Module):
    def __init__(self, num_features_in, num_anchors=9, num_classes=80, prior=0.01, feature_size=256):
        ...
        #self.free_anchor = nn.Conv2d(feature_size, num_classes, kernel_size=3, padding=1)
        self.base_anchor = nn.Conv2d(feature_size, num_anchors * num_classes, kernel_size=3, padding=1)
        self.act = nn.Sigmoid()
...
class ResNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        attention = [self.attentionModel(feature) for feature in features]
        ...
        if self.training:
            cls_loss, reg_loss = self.focalLoss(classification, regression, anchors, annotations)
            attention_loss = self.attentionLoss(img_batch.shape, attention, annotations)
            return cls_loss, reg_loss, attention_loss
        else:
